...
const consumedNutrientsSlice = createSlice({
	...
	reducers: {
		add(state, action) {
			const existingIndex = state.consumedNutrients.findIndex(
				nutrient => nutrient.id === action.payload.id
			)
			...
			if (existingIndex >= 0) {
				...
				const updatedNutrient = {
					...
					consumedQuantity:
						+existingNutrient.consumedQuantity + +action.payload.consumedQuantity,
		...
		delete(state, action) {
			const updatedConsumedNutrients = state.consumedNutrients.filter(
				nutrient => nutrient.id !== action.payload.id
			)

...
export default function DataSettings(props) {
	...
	const inputs = [
		{
			...
			placeholder: `${DCN} (kcal)`,
			type: 'number',
			initialValue: initialValues.dailyCalorieNeed,
		},
		{
			name: 'weight',
		...
		{
			name: 'fatRatio',
			placeholder: `${FAT_RATIO} (%)`,
			type: 'number',
			initialValue: initialValues.fatRatio,
		},
		{
			name: 'fitnessGoal',
			placeholder: FITNESS_GOAL,
			type: 'radio',
			initialValue: initialValues.fitnessGoal,
			selections,
		},
	]
	...
	const onSubmit = async values =>
		await submitHandler(UPDATE_SETTINGS_URL, PUT, values, updateSettings, null, onHide, HOME)
